...
export default function PdfToTxt() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(base + "/pdf-to-txt", {
        method: "POST",
        body: form,
      });
      ...
      console.error("PDF→TXT UI error:", err);
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.header}>
          <h1 style={styles.title}>PDF → TXT</h1>
          <p style={styles.subtitle}>Extract selectable text from a PDF — fast & private.</p>
        </div>
        ...
        <div style={styles.note}>
          Note: If your PDF is scanned (image-only), text may be empty. That needs OCR (paid tool).
        </div>
      </div>
    </div>
  );
