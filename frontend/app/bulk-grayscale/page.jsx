...
export default function BulkGrayscale() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(base + "/bulk-grayscale", {
        method: "POST",
        body: form,
      });

      if (!res.ok) {
        const txt = await res.text().catch(() => "Bulk grayscale failed");
        alert("Bulk grayscale failed:\n\n" + txt);
        return;
      }
      ...
      a.href = url;
      a.download = `bulk_grayscale_${Date.now()}.zip`;
      ...
      console.error("Bulk grayscale error:", err);
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.header}>
          <h1 style={styles.title}>Bulk Grayscale</h1>
          <p style={styles.subtitle}>
            Upload a ZIP of PDFs and download a ZIP of black & white PDFs — perfect for printing.
          </p>
        </div>
...
if (typeof window !== "undefined") {
  const styleId = "bulk-grayscale-loader-style";
