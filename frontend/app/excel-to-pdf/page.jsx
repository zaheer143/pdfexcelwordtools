...
export default function ExcelToPdf() {
  ...
  const convert = async () => {
    if (!file) return alert("Please upload an Excel file (.xls or .xlsx)");
    ...
    try {
      ...
      const res = await fetch(base + "/excel-to-pdf", {
        method: "POST",
        body: form,
      });
      ...
      const safeName =
        file.name.replace(/\.(xlsx?|XLSX?)$/, "") || "excel_to_pdf";
      a.href = url;
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.header}>
          <h1 style={styles.title}>EXCEL → PDF</h1>
          <p style={styles.subtitle}>
            Convert your Excel sheets (.xls, .xlsx) into clean PDF reports —
            fast &amp; private.
          </p>
        </div>
...
if (typeof window !== "undefined") {
  const styleId = "excel-to-pdf-loader-style";
