...
export default function BulkWatermarkPdf() {
  ...
  const convert = async () => {
    if (pdfs.length === 0) return alert("Upload at least one PDF");
    ...
    try {
      ...
      const res = await fetch(base + "/bulk-watermark-pdf", {
        method: "POST",
        body: form,
      });

      if (!res.ok) {
        const txt = await res.text().catch(() => "Bulk watermark failed");
        alert("Bulk watermark failed:\n\n" + txt);
        return;
      }
      ...
      a.href = url;
      a.download = `bulk_watermark_${Date.now()}.zip`;
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.header}>
          <h1 style={styles.title}>Bulk Watermark PDF</h1>
          <p style={styles.subtitle}>
            Upload multiple PDFs, add the same watermark to all, and download as a ZIP — fast & private.
          </p>
        </div>
        ...
        <div style={styles.uploadBox}>
          ...
          <div style={styles.fieldRow}>
            <div style={{ flex: 1 }}>
              <div style={styles.label}>Watermark text</div>
              <input
                value={watermarkText}
            ...
            <div style={{ width: 160 }}>
              <div style={styles.label}>Opacity ({Number(opacity).toFixed(2)})</div>
              <input
                type="range"
                min="0.05"
                max="0.60"
          ...
          <div style={styles.actions}>
            ...
            <button onClick={convert} style={styles.convertBtn} disabled={loading || pdfs.length === 0}>
              {loading ? <Loader /> : "Watermark & Download ZIP"}
            </button>
          </div>
...
function Loader() {
  return (
    <div style={styles.loaderWrap}>
      ...
      <span style={{ marginLeft: 8 }}>Watermarking...</span>
    </div>
  );
}
...
if (typeof window !== "undefined") {
  const styleId = "bulk-watermark-loader-style";
