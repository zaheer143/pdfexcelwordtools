...
export default function CompressToSize() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(base + "/compress-to-size", {
        method: "POST",
        body: form,
      });
      ...
      a.href = url;
      a.download = `${file.name.replace(/\.pdf$/i, "")}_compressed_${targetKb}kb.pdf`;
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.header}>
          <h1 style={styles.title}>Compress to Size</h1>
          <p style={styles.subtitle}>Compress a PDF to fit under a target size (best effort).</p>
        </div>
        ...
        <div style={styles.field}>
          <div style={styles.label}>Target size (KB)</div>
          <input
            type="number"
            min="50"
            step="50"
            value={targetKb}
            onChange={(e) => setTargetKb(Number(e.target.value || 500))}
            style={styles.input}
            disabled={loading}
          />
          <div style={styles.help}>Examples: 200 KB, 500 KB, 1024 KB (1 MB)</div>
        </div>
