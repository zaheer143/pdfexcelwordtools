...
export default function RemoveMetadata() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(base + "/remove-metadata", { method: "POST", body: form });

      if (!res.ok) {
        const txt = await res.text().catch(() => "Sanitize failed");
        alert("Sanitize failed:\n\n" + txt);
        return;
      }
      ...
      a.href = url;
      a.download = `${file.name.replace(/\.pdf$/i, "")}_sanitized.pdf`;
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.header}>
          <h1 style={styles.title}>Remove Metadata</h1>
          <p style={styles.subtitle}>Sanitize a PDF to remove hidden document info for privacy.</p>
        </div>
        ...
        <div style={styles.uploadBox}>
          ...
          <div style={styles.actions}>
            ...
            <button onClick={convert} style={styles.convertBtn} disabled={loading || !file}>
              {loading ? <Loader /> : "Remove Metadata"}
            </button>
          </div>
...
const styles = {
  ...
  thumb: { width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, color: "#2a0b62", background: "#e2f2ff", borderRadius: 8 },
  ...
  convertBtn: { flex: 1, padding: "12px 14px", borderRadius: 12, border: "none", background: "linear-gradient(90deg,#7b3cf5,#ff4db2)", color: "#fff", fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" },
...
if (typeof window !== "undefined") {
  const styleId = "remove-metadata-loader-style";
