...
export default function PdfRepair() {
  ...
  const repair = async () => {
    ...
    try {
      ...
      const res = await fetch(base + "/pdf-repair", {
        method: "POST",
        body: form,
      });

      if (!res.ok) {
        const txt = await res.text().catch(() => "Repair failed");
        alert("Repair failed:\n\n" + txt);
        return;
      }
    ...
    <div style={styles.page}>
      <div style={styles.card}>
        <div style={styles.header}>
          <h1 style={styles.title}>PDF Repair</h1>
          <p style={styles.subtitle}>
            Fix damaged or corrupted PDF files so they open correctly.
          </p>
        </div>
        ...
        <div style={styles.uploadBox}>
          ...
          <div style={styles.actions}>
            ...
            <button onClick={repair} style={styles.convertBtn} disabled={loading || !file}>
              {loading ? <Loader /> : "Repair PDF"}
            </button>
          </div>
...
function Loader() {
  return (
    <div style={styles.loaderWrap}>
      ...
      <span style={{ marginLeft: 8 }}>Repairing...</span>
    </div>
  );
}
...
if (typeof window !== "undefined") {
  const styleId = "pdf-repair-loader-style";
